function metadata = getOMEmetadata_nd2(omeMeta,metadata,fullfilepath)
...
for idxImage = 1:NBImage
    ...
    metadata(idxImage).AcquisitionDate = NaT;%datetime(omeMeta.getImageAcquisitionDate(0).getValue);
...
%     end
    metadata(idxImage).UnitZ = char(omeMeta.getPlanePositionZ(idxImage-1,0).unit.getSymbol);
    ...
    for idxF = 1:NBFrame
        ...
        x(idxF) = omeMeta.getPlanePositionX(idxImage-1,idxF-1).value;
        y(idxF) = omeMeta.getPlanePositionY(idxImage-1,idxF-1).value;

        zidx(idxF) = omeMeta.getPlaneTheZ(idxImage-1,idxF-1).getValue()+1;
        tidx(idxF) = omeMeta.getPlaneTheT(idxImage-1,idxF-1).getValue()+1;
        cidx(idxF) = omeMeta.getPlaneTheC(idxImage-1,idxF-1).getValue()+1;
    end
    t = t-t(1);
    zd = z-z(1);
    metadata(idxImage).TheC = cidx;
    metadata(idxImage).TheT = tidx;
    metadata(idxImage).TheZ = zidx;

    switch metadata(idxImage).TimeUnit
        case 's'
            metadata(idxImage).time = seconds(t);
    end
    metadata(idxImage).z = zd;
    if metadata(idxImage).DimT>1      
        metadata(idxImage).ResT = mean(diff(metadata(idxImage).time));
    end
    if metadata(idxImage).DimZ>1
         metadata(idxImage).ResZ = mean(diff(z));
    end

    metadata(idxImage).PosX = x;
    metadata(idxImage).PosY = y;
    metadata(idxImage).PosZ = z;
